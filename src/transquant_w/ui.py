"""Streamlit UI for the TransQuant probe weight factor (W) app.

``render()`` draws the full page body (everything the standalone ``app.py``
used to do, minus ``st.set_page_config``) so it can be embedded inside a host
Streamlit app that owns navigation, page config and the theme — e.g. via
``st.navigation``. See README.md, "Embedding in another Streamlit app".

``render()`` takes no parameters, does not call ``st.set_page_config``,
``st.navigation``, ``st.sidebar`` or ``st.stop``, and does not read or write
``st.session_state``, so it is safe to mount inside another app's page tree.
"""
...
def render() -> None:
    """Render the full TransQuant probe weight (W) app body."""
    ...
    with tcol2:
        target_file = st.file_uploader("…or upload FASTA", type=["fa", "fasta", "txt"], key="transquant_target_file")
    ...
    with pcol1:
        probes_text = st.text_area("Probe sequences (paste)", height=180, placeholder="1\tACGT...\t50\n2\t...",
                                   label_visibility="collapsed")
    with pcol2:
        probes_file = st.file_uploader("…or upload tab-delimited .txt", type=["txt", "tsv", "csv"], key="transquant_probes_file")
